...
if [ $# -ne 5 ]; then
  echo "Usage: $0 INTERVAL START END BASE_DIR LOG_DIR"
  exit 1
fi
...
for d in "$BASE_DIR"/global_step_*; do
  ...
  if (( step % INTERVAL == 0 && step >= START && step <= END )); then
    ...
    if [ -d "$ACTOR_DIR" ]; then
      ...
      echo "Launching step $step -> $ACTOR_DIR"
      nohup python scripts/model_merger.py \
        --local_dir "$ACTOR_DIR" \
        > "$LOG_FILE" 2>&1 &
    else
...
echo "所有符合条件（step ∈ [$START,$END] 且 % $INTERVAL == 0）的 merge 任务均已放入后台，日志保存在 $LOG_DIR。"
